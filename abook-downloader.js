...
(async function () {
	...
	let stringWidth = await import("string-width");
	...
	let getResourceUnitInfo = async function (session, courseInfoId, resourceStructure, downloadLinks, retry = defaultRetry.bind(99999)) {
		...
		if (resourceInfo === "needDesktop") {
			...
			for (let i of resourceInfo) {
				i.parentId = resourceStructure.id;
	...
	let getResourceInfo = async function (session, courseInfoId, resourceStructure, downloadLinks, log = print, retry = defaultRetry) {
		...
		if (resourceStructure.type === 1) {
			...
			if (temp[0]) {
				print("Failed to fetch resource information of resource %s. Won't retry.".format(resourceStructure.id));
	...
	let sortResourceInfo = function sortResourceInfo(resourceStructure, resourceInfo, resourceInfoByParentId) {
		let ret = [];
		if (!resourceInfoByParentId) {
			resourceInfoByParentId = {};
			for (let i of resourceInfo) {
				if (!resourceInfoByParentId[i.parentId]) {
					resourceInfoByParentId[i.parentId] = [];
				}
				resourceInfoByParentId[i.parentId].push(i);
			}
		}
		if (resourceInfoByParentId[resourceStructure.id]) {
			ret = ret.concat(resourceInfoByParentId[resourceStructure.id]);
		}
		for (let i of resourceStructure.children) {
			ret = ret.concat(sortResourceInfo(i, resourceInfo, resourceInfoByParentId));
		}
		return ret;
	};
	...
	let getCourseResourceInfo = async function (session, courseInfoId, resourceId, read = input, log = print, retry = defaultRetry) {
		...
		return [false, sortResourceInfo(resourceStructure[2][resourceId], resourceInfo[1])];
	};
	let curlDownloadScript = function (resourceInfoList, pathBase) {
		let ret1 = "", ret2 = "", dirs = {};
	...
	let aria2DownloadList = function (resourceInfoList, pathBase) {
		let ret = "";
		for (let resourceInfo of resourceInfoList) {
			let path = pathBase.concat(resourceInfo.path)
			path = path.concat([validateFilename(resourceInfo.resTitle + "." + resourceInfo.format)]);
			ret += "https://abook.hep.com.cn/ICourseFiles/" + resourceInfo.resFileUrl + "\n  out=" + path.join("/") + "\n";
		}
		return ret;
	};
	...
	(async function main() {
		...
		while (true) {
			...
			let download = async function (session, resourceInfo, path) {
				while (true) {
					let choice = (await input("Downloader (Built-in - default, Aria2, cUrl, Cancel): ")).toLowerCase(), time = Date.now();
					if (choice === "b" || choice === "") {
						await downloadResource(session, resourceInfo, path);
					} else if (choice === "a") {
						fs.writeFileSync("AbookAria2DownloadList" + time, aria2DownloadList(resourceInfo, path));
						print("aria2 download list saved to AbookAria2DownloadList" + time + " .");
					} else if (choice === "u") {
						fs.writeFileSync("AbookCurlDownloadScript" + time + ".sh", curlDownloadScript(resourceInfo, path));
						print("curl download script saved to AbookAria2DownloadList" + time + ".sh .");
					} else if (choice === "c") {
			...
			while (true) {
				...
				if (choice === "a") {
					...
					for (let i of range(courseList.len)) {
						...
						let prepDone = Symbol();
						for (let j of resourceInfo[1]) {
							if (!j.path[prepDone]) {
								j.path.unshift(validateFilename(courseList[i][1]));
								j.path[prepDone] = true;
							}
						}
						allResourceInfo = allResourceInfo.concat(resourceInfo[1]);
					}
					await download(session, allResourceInfo, ["AbookDownloads"]);
					printCoursesList(courseList);
					continue;
				}
				if (choice === "r") {
				...
				if (courseList[choice]) {
					...
					if (!resourceInfo[0]) {
						await download(session, resourceInfo[1], ["AbookDownloads", validateFilename(courseList[choice][1])]);
						printCoursesList(courseList);
					}
					continue;
				}
